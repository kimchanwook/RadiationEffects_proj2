...
\begin{itemize}[leftmargin=*]
  ...
  \item No electrostatic, thermal, defect, carrier, or coupled solve is called by the Module 9 driver.
  ...
  \item There is no visual regression test. A person must inspect the two generated figures.
\end{itemize}
...
\matlabfile{matlab/tests/test_module9_transmon_geometry_2d.m} & Master Module 9 test runner. Calls the four focused regression tests in a fixed order. This is the normal test command. \\
\bottomrule
\end{longtable}
...
\begin{enumerate}[leftmargin=*]
  ...
  \item \matlabfile{test_module9_geometry_tags_2d.m}; and
  \item \matlabfile{test_module9_geometry_no_wire_arc_2d.m}.
\end{enumerate}
...
\subsection{Gate 2: connect Module 2 FEM}
...
\begin{longtable}{>{\RaggedRight\arraybackslash}p{0.43\textwidth} >{\RaggedRight\arraybackslash}p{0.48\textwidth}}
\toprule
\textbf{Existing or new location} & \textbf{Required change} \\
\midrule
\endfirsthead
\toprule
\textbf{Existing or new location} & \textbf{Required change} \\
\midrule
\endhead
\matlabfile{default_module2_params.m} & Add legacy-rectangle versus Module 9 geometry selection and tagged-electrode BC settings. \\
\matlabfile{solve_poisson_defect_space_charge_2d.m} & Accept a supplied mesh/geometry instead of always constructing the old rectangle. \\
\matlabfile{get_module2_dirichlet_nodes.m} & Consume \texttt{left\_electrode} and \texttt{right\_electrode} tags rather than whole chip sides. \\
\matlabfile{plot_module2_result_2d.m} & Overlay Module 9 tags and label coordinates $(x,z)$. \\
\matlabfile{main_module2_electrostatics.m} and \matlabfile{matlab/cases/} & Add a named transmon trapped-charge case without changing existing cases. \\
\matlabfile{assemble_poisson_fem_2d.m} & Extend scalar permittivity only when nodal or elementwise material coefficients are actually needed. \\
\matlabfile{matlab/tests/} & Add Module 2-on-Module 9 tests for tag BCs, electrode separation, source sign, field direction, and legacy regression. \\
\bottomrule
\end{longtable}

\subsection{Gate 3: first tagged thermal validation}
...
\subsection{Gate 4: propagate one shared mesh through Modules 3--6}
...
\subsection{Gate 5: adapt the PINN last}
...
\begin{notebox}
\textbf{Immediate next task after geometry acceptance.} Connect Module 2 FEM to the shared Module 9 mesh and the named left/right electrode tags while preserving all legacy rectangular tests. Do not begin the PINN conversion at the same time.
\end{notebox}
