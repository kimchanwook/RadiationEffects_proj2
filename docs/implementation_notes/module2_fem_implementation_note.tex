...
\begin{overviewbox}
\textbf{Purpose and scope.} This is the operational companion to the Module 2 physics note. It documents the MATLAB finite-element method (FEM) implementation of the two-dimensional electrostatic Poisson problem on both the original rectangular verification domain and the validated Module 9 transmon substrate mesh. It also documents the new arbitrary nodal charge-field input contract and the factorized multi-configuration FEM dataset path beginning at \matlabfile{main_module2_batch_fem_dataset.m}. The canonical Module 2 input is now the complete nodal total-charge field $\bm{\rho}$; Gaussian parameters are retained only as synthetic field-generator metadata. It explains what to run, how named electrode tags are consumed, which files are called, what each returned field means, how to execute all legacy, transmon, and batch verification tests, and how to diagnose an unexpected result. The separate PINN guide remains authoritative for the neural-network path.
\end{overviewbox}

\begin{notebox}
\textbf{Fastest correct start.} Open MATLAB in the repository's \matlabfile{matlab/} directory, run \texttt{setup\_project\_paths}, then run \texttt{test\_module2\_fem\_all\_2d}. After the suite passes, generate the two geometry-aware demonstrations with \texttt{main\_module2\_electrostatics('transmon\_laplace')} and \texttt{main\_module2\_electrostatics('transmon\_trapped\_charge')}. Then generate the field-to-field surrogate pilot data with \texttt{[dataset,report] = main\_module2\_batch\_fem\_dataset()}. The batch MAT-file and validation summary are written under \matlabfile{matlab/outputs/module2_batch_fem_2d/}; plots remain disabled by default.
\end{notebox}
...
$\bm{\rho}$ & Nodal charge-field input & One total charge-density value per FEM node; canonical Module 2 input. \\
$\bm{\mu}_{\mathrm{gen}}$ & Synthetic-generator metadata & $[C_{\mathrm{peak}},x_c,z_c,\sigma_x,\sigma_z]$ for the default Gaussian pilot only. \\
...
\begin{itemize}[leftmargin=*]
  ...
  \item Explicit arbitrary nodal total-charge fields supplied directly to the solver.
  \item Composition of scalar or nodal electron, hole, ionized-dopant, and charged-defect fields into total $\rho$.
  \item Gaussian and uniform charge generators retained as synthetic verification/data-generation utilities.
  ...
  \item Deterministic 17-field pilot dataset storing complete $\bm{\rho}\rightarrow\bm{\Phi}$ pairs, with one stiffness assembly, one sparse Cholesky factorization, and complete-field train/validation/test splits.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
  ...
  \item The solver does not solve carrier statistics or trap occupancy self-consistently. It accepts an externally prepared total charge field, or prescribed component fields that are combined before the Poisson solve.
...
\matlabfile{matlab/src/module2_electrostatics/default_module2_params.m} & Defines constants, geometry mode, synthetic demonstration parameters, side/tag boundary conditions, output controls, and supported named cases. \\
\matlabfile{matlab/src/module2_electrostatics/make_module2_charge_field_2d.m} & Packages and validates the canonical arbitrary nodal total-charge field. \\
\matlabfile{matlab/src/module2_electrostatics/validate_module2_charge_field_2d.m} & Validates the field schema, nodal count, representation, units, and finite values. \\
\matlabfile{matlab/src/module2_electrostatics/compose_module2_charge_field_2d.m} & Combines scalar or nodal $p,n,N_D^+,N_A^-,C_{\mathrm{def}},z_{\mathrm{def}}$ fields into total $\rho$. \\
\matlabfile{matlab/src/module2_electrostatics/generate_module2_gaussian_charge_field_2d.m} & Synthetic Gaussian/uniform field generator for controlled cases; not the canonical solver interface. \\
\matlabfile{matlab/src/module2_electrostatics/build_space_charge_module2_2d.m} & Legacy compatibility wrapper retained for the current pointwise PINN prototype and old scripts. \\
\matlabfile{matlab/src/module2_electrostatics/solve_poisson_defect_space_charge_2d.m} & Numerical orchestrator. Accepts an explicit nodal charge field, then performs assembly, boundary processing, sparse solve, and field postprocessing. \\
\matlabfile{matlab/src/module2_electrostatics/solve_module2_nodal_charge_batch_2d.m} & Generic multi-field solver that reuses one operator/factorization for arbitrary $\bm{\rho}$ columns. \\
...
\matlabfile{matlab/src/module2_electrostatics/make_module2_gaussian_field_pilot_design_2d.m} & Defines deterministic Gaussian-generator metadata for the 17-field pilot and complete-field split indices. \\
\matlabfile{matlab/src/module2_electrostatics/make_module2_batch_parameter_design_2d.m} & Legacy wrapper exposing the old parameter-field names for compatibility. \\
...
\begin{enumerate}[leftmargin=*]
  ...
  \item If no explicit charge field is supplied, the built-in synthetic generator creates one nodal $\bm{\rho}$ field. Otherwise the supplied field is validated and used directly.
...
The v2 batch contract is field-to-field:
\begin{equation}
\boxed{\bm{\rho}^{(m)}\longrightarrow\bm{\Phi}^{(m)}}.
\end{equation}
The default pilot still creates these input fields from one elliptical Gaussian effective-defect cloud with generator metadata
\begin{equation}
\bm{\mu}_{\mathrm{gen}}=\left[C_{\mathrm{peak}},x_c,z_c,\sigma_x,\sigma_z\right].
\end{equation}
The deterministic generator design contains a zero control and 16 charged fields spanning amplitude, depth, lateral displacement, and anisotropic width. Its 11 training, three validation, and three test entries are indices of \emph{complete field configurations}; nodes from a held-out field are never placed in the training set. The five values in $\bm{\mu}_{\mathrm{gen}}$ are provenance only and are not the canonical neural-surrogate inputs.
...
\begin{enumerate}[leftmargin=*]
  ...
  \item generate or import one complete nodal $\bm{\rho}$ field for each case, assemble only its source vector, apply the fixed-value right-hand-side correction, and solve $L\mathbf{y}=\mathbf{b}_{\mathrm{bc}}$ followed by $L^T\phivec=\mathbf{y}$;
...
The saved v2 structure stores \texttt{mesh} and \texttt{tags} once. The matrices \texttt{rho} and \texttt{phi} contain one row per mesh node and one column per complete field case; these two matrices are the canonical surrogate input/output pair. The Gaussian generator parameters are stored only under \texttt{syntheticGenerator} as provenance metadata. The fields \texttt{trainCases}, \texttt{validationCases}, and \texttt{testCases} identify whole field columns. The stiffness matrix, Cholesky factor, and full residual vectors are intentionally omitted from the saved training dataset.

\begin{notebox}
\textbf{Linearity decision.} The pilot keeps both electrode voltages fixed at zero. For the current linear Poisson equation, nonzero contact-voltage contributions can later be supplied exactly through precomputed Laplace basis solutions and superposition. The first field-to-field surrogate therefore focuses on the charge-field response without wasting capacity on a contribution already known analytically.
\end{notebox}
...
\subsection{Canonical charge-field input and synthetic source controls}

The preferred direct-solver call supplies an explicit nodal field:
\begin{lstlisting}[style=projectmatlab]
[mesh,~,~] = resolve_module2_mesh_2d(params);
rho = ... % one C/m^3 value per mesh node
chargeField = make_module2_charge_field_2d(mesh, rho);
result = solve_poisson_defect_space_charge_2d(params, chargeField);
\end{lstlisting}
A raw numeric nodal \texttt{rho} vector is also accepted as the second solver argument. For coupled use, \texttt{compose\_module2\_charge\_field\_2d} accepts scalar or nodal component fields and applies the semiconductor sign convention before solving.

The scalar fields below remain available because the built-in named cases and Gaussian pilot generator use them. They are synthetic/demo controls, not a restriction on the solver input.
...
When the synthetic Gaussian generator is selected and \texttt{rho\_uniform} does not exist, it evaluates
\begin{align}
C_{\mathrm{def}}(x,y)
...
\begin{notebox}
\textbf{Synthetic-source pitfall.} The fields above control only the built-in synthetic generator. If an explicit \texttt{chargeField} or numeric \texttt{rho} vector is passed to the solver, that nodal field is authoritative and the Gaussian source controls do not modify it.
\end{notebox}
...
\begin{itemize}[leftmargin=*]
  \item \matlabfile{module2_transmon_field_to_field_gaussian_pilot_v2.mat}: current complete field-to-field dataset structure;
  \item \matlabfile{module2_transmon_field_to_field_gaussian_pilot_v2_summary.txt}: current case manifest, split membership, operator reuse counts, and worst diagnostics;
  \item older \matlabfile{module2_transmon_gaussian_batch_v1.*} files may remain as historical pre-v2 outputs;
...
size(loaded.dataset.rho)          % expected: nNodes by 17
size(loaded.dataset.phi)          % expected: nNodes by 17
loaded.dataset.syntheticGenerator.parameters  % provenance only: 17 by 5
...
\end{lstlisting}
This master runner executes the legacy analytical/sanity tests, the new explicit-field and component-field contract tests, the focused Module 2-on-Module 9 integration tests, and the batch-dataset suite. For a focused diagnosis, use:
\begin{itemize}[leftmargin=*]
...
\matlabfile{test_module2_explicit_charge_field_2d} & Arbitrary non-Gaussian nodal $\rho$ is preserved exactly and produces a converged solve & The canonical solver path no longer requires Gaussian parameters. \\
\matlabfile{test_module2_charge_component_fields_2d} & Spatial/scalar component fields reproduce the signed semiconductor charge formula exactly & $p,n,N_D^+,N_A^-,C_{\mathrm{def}},z_{\mathrm{def}}$ can be assembled into total nodal $\rho$. \\
...
\matlabfile{test_module2_batch_dataset_contract_2d} & All aggregate validation checks pass & The v2 dataset stores mandatory nodal $\rho$ input and nodal $\phi$ output fields, complete-field splits, tags, diagnostics, and Gaussian provenance only under \texttt{syntheticGenerator}. \\
...
\matlabfile{test_module2_batch_split_integrity_2d} & Splits are disjoint and cover every case once & No complete-field leakage is introduced before field-surrogate training. \\
\matlabfile{test_module2_batch_reproducible_design_2d} & Repeated generator-design construction is identical & The 17-field Gaussian pilot generator is deterministic and auditable. \\
\bottomrule
\end{longtable}
...
\section{Preparing for Module 3 and Module 5 coupling}

The field interface required for coupling is now implemented. Upstream modules may provide one or more nodal concentration fields and Module 2 may compose them as
\begin{equation}
\bm{\rho}=q\left(\bm{p}-\bm{n}+\bm{N}_D^+-\bm{N}_A^-+\sum_i \bm{z}_i\odot\bm{C}_i\right).
\end{equation}
The helper \matlabfile{compose_module2_charge_field_2d.m} already accepts scalar or nodal values for the current reduced set $p,n,N_D^+,N_A^-,C_{\mathrm{def}},z_{\mathrm{def}}$. The resulting \texttt{module2\_charge\_field\_v1} structure can be passed directly to the FEM solver.

Before connecting live Module 3 or Module 5 outputs, still define and test:
\begin{itemize}[leftmargin=*]
  \item whether the modules share exactly the same node ordering and mesh;
  \item the interpolation/conservative-projection rule when their meshes differ;
  ...
  \item behavior at times when one component field is unavailable;
  \item whether an upstream module supplies components separately or an already combined total-charge map.
\end{itemize}

The key architectural change is that no Gaussian fit is required between modules. A physically generated spatial field can be passed through the same validated $\bm{\rho}$ contract used by synthetic tests.
...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{Field contract:} run the two new field-interface tests.\par
  {\small\texttt{test\_module2\_explicit}\,\texttt{\_charge\_field\_2d}}\par
  {\small\texttt{test\_module2\_charge}\,\texttt{\_component\_fields\_2d}}.
...
\begin{itemize}[leftmargin=*]
  ...
  \item for batch data, field-dataset schema, complete-field split indices, synthetic-generator provenance when applicable, operator/factorization counts, and worst per-case diagnostics;
...
[dataset, report] = main_module2_batch_fem_dataset();
\end{lstlisting}
The next coding gate is to replace the executable pointwise Module 2 PINN with the field-to-field architecture defined in the revised physics note. The neural model should consume the saved nodal $\bm{\rho}$ field and predict nodal $\bm{\Phi}$ on the accepted geometry. For physics-informed training, the preferred baseline is the differentiable discrete FEM residual $K_{FF}\bm{\Phi}_{\theta,F}+K_{FD}\bm{\Phi}_D-\bm{b}_F(\bm{\rho})$, with complete-field train/validation/test splits and the normalized/adaptive loss weighting described in the physics note. The current \matlabfile{module2_pinn_implementation_note.pdf} remains a guide to the legacy pointwise prototype until that code migration is completed. Do not scale to hundreds of fields until the v2 field contract and a small held-out end-to-end neural pilot pass.
