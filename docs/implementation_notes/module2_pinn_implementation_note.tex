...
\begin{notebox}
\textbf{Architecture migration notice (2026-08-19).} The project-level Module 2 contract has changed to field-to-field learning, $\bm{\rho}\mapsto\bm{\Phi}_{\theta}$. The FEM/data pipeline already accepts arbitrary nodal charge fields and writes v2 complete-field datasets. The executable files documented in this implementation note still implement the older pointwise single-case PINN, $(x,y)\mapsto\phi_{\theta}(x,y)$, and are retained as regression/prototype code until the neural implementation is migrated. Therefore this document is authoritative for how the \emph{current legacy PINN code} runs, while the revised \matlabfile{module2_PINN.pdf} physics note is authoritative for the new target architecture.
\end{notebox}

\begin{notebox}
\textbf{Fastest safe start for the legacy prototype.} Open MATLAB in \matlabfile{RadiationEffects_proj2/matlab/}, run \texttt{setup\_project\_paths}, confirm the Deep Learning Toolbox is available, run \texttt{test\_module2\_fem\_all\_2d}, and then run \texttt{test\_module2\_pinn\_electrostatics}. After those preflight checks, train the analytical linear-potential case before the Gaussian defect case.
\end{notebox}
...
Mode B & One supervised field operator learns $\bm{\rho}\mapsto\bm{\Phi}$ from many complete FEM field pairs. & New target baseline; FEM v2 data contract implemented, neural code not yet implemented. \\
Mode C & One physics-informed field operator learns $\bm{\rho}\mapsto\bm{\Phi}$ using FEM data plus the discrete FEM residual $K\bm{\Phi}_{\theta}-\bm{b}(\bm{\rho})$ and boundary constraints. & Final target; physics design documented, neural code not yet implemented. \\
\bottomrule
\end{longtable}

\begin{physicsbox}
\textbf{Interpretation rule.} The current executable PINN code is an implementation and verification benchmark for pointwise PINN mechanics. It is not the newly selected field-to-field surrogate. Training a new pointwise network for every FEM case may cost more than solving that case directly with FEM; the amortized target is one operator trained over many complete $\bm{\rho}\rightarrow\bm{\Phi}$ pairs.
\end{physicsbox}
...
\section{Migration target: field-to-field Module 2 surrogate}
...
\begin{equation}
\boxed{\mathcal{G}_{\theta}:\bm{\rho}\mapsto\bm{\Phi}_{\theta}}.
\end{equation}
The v2 FEM dataset already stores \texttt{rho} and \texttt{phi} as $N_{\mathrm{nodes}}\times N_{\mathrm{cases}}$ arrays. Gaussian values such as $(C_{\mathrm{peak}},x_c,z_c,\sigma_x,\sigma_z)$ are stored only under synthetic-generator provenance and must not be used as the canonical neural inputs.

The target implementation therefore requires:
\begin{itemize}[leftmargin=*]
  \item a field architecture appropriate for either a structured raster (CNN/U-Net/FNO) or the existing triangular mesh (graph/mesh operator);
  \item complete-field minibatches and complete-field train/validation/test splits;
  \item normalization of $\bm{\rho}$ and $\bm{\Phi}$ using training-set scales only;
  \item hard or soft treatment of tagged Dirichlet electrode values;
  \item a differentiable discrete physics residual on free FEM degrees of freedom,
  \begin{equation}
  \bm{r}_{\mathrm{PDE}}=K_{FF}\bm{\Phi}_{\theta,F}+K_{FD}\bm{\Phi}_D-\bm{b}_F(\bm{\rho});
  \end{equation}
  \item normalized data/PDE/boundary losses followed by adaptive gradient-balanced weighting when needed;
  \item held-out non-Gaussian fields in addition to Gaussian pilot fields;
  \item amortized inference-time comparison against FEM;
  \item saved mesh, normalization, geometry, boundary, and training-domain metadata with the network.
\end{itemize}
...
\begin{notebox}
\textbf{Do not overstate the current result.} A successful run of the files documented in the preceding chapters demonstrates the older pointwise PINN mechanics only. Generalization to arbitrary unseen charge maps and field-to-field surrogate speedup must be demonstrated after the migration above.
\end{notebox}
...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{Reference FEM:} pass \texttt{test\_module2\_fem\_all\_2d}, including the explicit-field and component-field contract tests.
