...
\section{Canonical spatial-field input contract}
...
\end{equation}
with one value in C/m$^3$ at every active mesh node.
...
\bm{\rho}
=q\left(
\bm{p}-\bm{n}+\bm{N}_D^+-\bm{N}_A^-+
\sum_i \bm{z}_i\odot\bm{C}_i
\right).
}
\label{eq:nodal_charge_contract}
\end{equation}
...
This means Module 2 does \emph{not} require a list of individual defect coordinates, nor does it require that the charge field be Gaussian. Microscopic defect information must first be coarse-grained into continuum concentrations or total charge density on the Module 2 mesh. Once $\bm{\rho}$ has been formed, the electrostatic solver only needs that field and the material/boundary data.

\subsection*{Role of the Gaussian model}
...
\end{equation}
Those five parameters are useful for producing reproducible verification and training cases, but they are no longer the Module 2 input contract. Imported non-Gaussian fields, multi-cluster fields, and future Module 3/Module 5 fields all enter through the same $\bm{\rho}$ interface.
...
\texttt{default\_module2\_params.m} & Defines physical constants, geometry defaults, synthetic demonstration parameters, and boundary conditions. \\
\texttt{make\_module2\_charge\_field\_2d.m} & Packages and validates the canonical arbitrary nodal total-charge field. \\
\begin{tabular}[t]{@{}l@{}}\texttt{compose\_module2\_charge}\\\texttt{\_field\_2d.m}\end{tabular} & Combines scalar or nodal $p,n,N_D^+,N_A^-,C_{\mathrm{def}},z_{\mathrm{def}}$ inputs into total nodal $\rho$. \\
\begin{tabular}[t]{@{}l@{}}\texttt{generate\_module2\_gaussian}\\\texttt{\_charge\_field\_2d.m}\end{tabular} & Generates a controlled Gaussian-based synthetic charge field for tests and dataset construction. \\
\texttt{build\_space\_charge\_module2\_2d.m} & Legacy compatibility wrapper around the synthetic Gaussian field generator. \\
...
\begin{tabular}[t]{@{}l@{}}\texttt{solve\_poisson\_defect}\\\texttt{\_space\_charge\_2d.m}\end{tabular} & Accepts an explicit nodal charge field, then orchestrates assembly, boundary handling, linear solve, and field calculation. \\
\begin{tabular}[t]{@{}l@{}}\texttt{solve\_module2\_nodal}\\\texttt{\_charge\_batch\_2d.m}\end{tabular} & Reuses one FEM operator and factorization for many arbitrary $\bm{\rho}\rightarrow\bm{\Phi}$ field pairs. \\
