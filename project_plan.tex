...
6 & Coupled two-dimensional multiphysics degradation model & Couple Modules 2, 3, 4, and 5 into a self-consistent simulator for defect evolution, field evolution, thermal evolution, and carrier-transport degradation. The current implementation adds a first staggered shared-mesh linear-triangle FEM coupling scaffold with convergence and charge-consistency diagnostics.\\
...
\textbf{Module 8. Reduced-Fidelity and Importance-Based Physics Selection}
...
\textbf{Module 9. Transmon Geometry, Microwave Package, and 2D FEM Domains}
...
\textbf{Module 10. BCS, Ginzburg--Landau, and Time-Dependent Ginzburg--Landau Theory}
...
\textbf{Module 11. Graph Neural Networks for Physical Dependency Discovery}
...
box/.style={draw=accent, rounded corners=2pt, fill=accent2, minimum width=0.87\linewidth, text width=0.87\linewidth, align=left, inner sep=4pt, font=\small},
arrow/.style={-{Latex[length=2.4mm]}, very thick, draw=accent}
]
\node[box] (m1) {\textbf{Step 1. Module 1: Geant4 radiation transport}\\Score deposited energy, hit coordinates, timing, interactions, and damage-source products.};
\node[box, below=of m1] (handoff) {\textbf{Step 2. Geant4-to-MATLAB handoff}\\Map Geant4 output to a 2D deposition or initial defect-source field.};
\node[box, below=of handoff] (m3) {\textbf{Step 3. Module 3: defect evolution}\\Evolve defects under diffusion, reactions, annealing, and source terms.};
\node[box, below=of m3] (m4) {\textbf{Step 4. Module 4: ballistic-diffusive thermal field}\\Update the ballistic-diffusive temperature field and temperature-dependent coefficients.};
\node[box, below=of m4] (m2) {\textbf{Step 5. Module 2: electrostatics}\\Build space charge and solve for potential and electric field.};
\node[box, below=of m2] (m5) {\textbf{Step 6. Module 5: carrier transport}\\Solve electron and hole drift, diffusion, recombination, and trapping.};
\node[box, below=of m5] (m6) {\textbf{Step 7. Module 6: coupled iteration}\\Iterate Modules 2--5 to a self-consistent coupled state.};
\node[box, below=of m6] (m11) {\textbf{Step 8. Module 11: physical graph study}\\Generate graph-labeled trajectories and recover directed physical dependencies, lags, and feedback groups.};
\node[box, below=of m11] (m7) {\textbf{Step 9. Module 7: scalable prediction}\\Extend predictions using reduced-order, statistical, or multiresolution methods.};
...
\begin{enumerate}[leftmargin=1.6em]
    ...
    \item \textbf{Module 3:} run the implemented pure-annealing causal-PINN regression benchmark, verify convergence with a production training budget, then add the matched ordinary-PINN and Gaussian-diffusion comparison.
    \item \textbf{Module 2:} 2D electrostatics with arbitrary prescribed nodal total-charge fields; the FEM path and v2 $\bm{\rho}\rightarrow\bm{\Phi}$ batch contract are implemented and ready for direct coupling to Module 3/5 fields. The executable PINN remains the older pointwise prototype and is the next migration target.
    ...
    \item \textbf{Module 5:} 2D drift-diffusion with prescribed electrostatic field, thermal field, and defect/trap fields; first linear-triangle FEM path now implemented.
    \item \textbf{Module 6:} weak coupling, then stronger feedback and iterative convergence.
    \item \textbf{Module 7/8:} scalable-prediction methods and impact-per-cost physics selection after at least one credible high-fidelity coupled workflow exists.
    \item \textbf{Module 9:} transmon geometry and 2D FEM domain tagging for the superconducting extension.
    \item \textbf{Module 10:} use the BCS/GL/TDGL note as the theory basis for superconducting constitutive models and device metrics.
    \item \textbf{Module 11:} define the candidate physical-variable graph, generate multiple graph families and matched interventions, establish non-graph structure-learning baselines, then train a typed GNN for directed edge, relation, strength, lag, and feedback-group recovery.
\end{enumerate}
...
\path{docs/physics_notes/module3_cPIN.tex/.pdf} & Introductory causal-PIN tutorial, implemented first MATLAB benchmark, and extension design for temporally ordered defect diffusion-reaction training and validation.\\
...
\path{matlab/} & MATLAB drivers, source code, tests, outputs, and cross-module interfaces for Modules 2--7; Module 9 supplies the next geometry extension and Module 11 supplies the planned physical-dependency graph data and learning workflow.\\
\bottomrule
\end{longtable}
...
\path{matlab/main_module3_2d_defect_evolution.m} & Top-level entry for the structured-grid 2D defect-evolution solve.\\
\path{matlab/main_module3_fem_defect_evolution.m} & Top-level entry for the linear triangular FEM defect diffusion-reaction solve.\\
\path{matlab/main_module3_CPINN_Defect_Evolution.m} & Top-level causal-PINN driver with dimensionless automatic-differentiation residuals, ordered physical-time slices, stop-gradient causal weights, analytic/FEM references, and training diagnostics.\\
\path{matlab/main_module4_2d_ballistic_diffusive_thermal.m} & Top-level entry for the structured-grid Module 4 two-dimensional ballistic-diffusive thermal solve.\\
\path{matlab/main_module4_fem_ballistic_diffusive_thermal.m} & Top-level entry for the linear triangular FEM Module 4 ballistic-diffusive thermal solve.\\
\path{matlab/main_module5_drift_diffusion.m} & Top-level entry for the 2D FEM carrier-transport solve.\\
\path{matlab/main_module5_fem_drift_diffusion.m} & Compatibility wrapper for the Module 5 linear triangular FEM carrier solver.\\
\path{matlab/main_module6_multiphysics.m} & Top-level coupled FEM driver that orchestrates Modules 2, 3, 4, and 5 through a staggered Picard-style iteration.\\
\path{matlab/main_module11_generate_dependency_dataset.m} & Planned Module 11 driver for graph-family sampling, constrained physical trajectories, matched interventions, graph labels, and dataset export.\\
\path{matlab/main_module11_train_dependency_gnn.m} & Planned Module 11 driver for structure-learning baseline comparison, directed typed graph-model training, calibration, and held-out evaluation.\\
\path{matlab/src/module11_gnn/} & Planned variable-graph construction, graph-family sampling, intervention generation, feature extraction, directed edge/type/lag losses, SCC sequencing, uncertainty, and evaluation utilities.\\
\path{matlab/src/grid/} & Shared 2D grid generation and spatial operators.\\
\path{matlab/src/module3_defects/} & Module 3 defect evolution operators, initialization helpers, material-aware coefficients, and the new FEM mass/diffusion/reaction assembly functions.\\
\path{matlab/src/module4_thermal/} & Module 4 ballistic-diffusive thermal operators, initialization helpers, source-term assembly, reduced ballistic-flux closures, FEM heat-capacity and conductivity assembly, implicit FEM stepping, and verification support.\\
\path{matlab/src/module2_electrostatics/} & Implemented Module 2 finite-element Poisson operators, arbitrary nodal charge-field validation/composition, synthetic Gaussian field generation, reusable field-batch solves, triangular mesh generation, boundary-condition insertion, and field postprocessing.\\
\path{matlab/src/module5_transport/} & Module 5 FEM drift-diffusion operators, carrier matrix assembly, linear recombination closure, mobility/source evaluation, current extraction, plotting, and summary output.\\
...
\path{matlab/tests/} & Verification problems and regression tests, including Module 2 electrostatic limiting-case checks, the Module 3 causal-PINN pure-annealing regression/physics test, Module 3 FEM diffusion-reaction checks, Module 4 FEM thermal checks, and Module 5 FEM carrier-transport checks.\\
...
\section{Module 11: physical dependency graph proof-of-concept plan}
...
\begin{enumerate}[leftmargin=1.6em]
    \item freeze the variable-node list, edge taxonomy, candidate directed-edge mask, sign priors, and allowed lag classes;
    \item expose key Module 2--5 mechanisms through explicit graph switches and continuous edge multipliers;
    \item generate constrained physical parameters, sources, initial conditions, and multiple graph families;
    \item create matched hard, soft, node-clamp, and source interventions;
    \item extract variable-node features from raw physical trajectories without using spatial adjacency as graph topology;
    \item establish correlation, conditional, Granger-style, sparse-regression, MLP, fixed-graph, and NRI baselines;
    \item train a directed typed message-passing network for state prediction, edge existence, relation type, strength, sign, lag, and uncertainty;
    \item identify strongly connected components and topologically order the condensation graph to obtain a physical coupling sequence;
    \item test noise, missing variables, held-out graph combinations, and hidden-mechanism discrepancy.
\end{enumerate}
...
\begin{enumerate}[leftmargin=1.6em]
    ...
    \item Verify the structured-grid and FEM Module 3 code paths, the FEM Module 4 code path, and the new FEM Module 5 carrier-transport code path, on the user's MATLAB environment.
    \item Completed first stage: implemented the Module 3 causal-PINN pure-annealing/Gaussian driver and exact pure-annealing regression test described in \path{module3_cPIN.tex}. Next add a matched ordinary-PINN mode, a longer Gaussian convergence test, and an imported Geant4-like map.
    \item Completed: implemented the first 2D reduced ballistic-diffusive Module 4 structured-grid MATLAB path with uniform-equilibrium, localized-pulse, and boundary-heating cases.
    \item Keep the Module 4 note, file map, and run-order documentation synchronized with both the structured-grid and FEM thermal paths.
    \item Completed: added Module 2, Module 3, Module 4, and Module 5 FEM implementation notes and first reduced FEM solver paths.
    \item Freeze the first Module 11 physical-variable node list, candidate directed-edge mask, relation taxonomy, and intervention definitions.
    \item Build the first 500--2,000-case graph-family synthetic dataset with explicit mechanism switches and matched interventions.
    \item Establish non-graph structure-learning baselines and a fixed-graph predictive baseline before training a directed typed GNN.
